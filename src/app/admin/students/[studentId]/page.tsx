...
const MetricSlider = ({ name, label }: { name: `metrics.${keyof Omit<EvaluationMetrics, 'overall'>}`, label: string }) => (
    <FormField control={useForm<EvaluationFormValues>().control} name={name} render={({ field }) => (
        <FormItem>
            <FormLabel>{label} - {field.value}/5</FormLabel>
            <FormControl><Slider defaultValue={[field.value]} onValueChange={(value) => field.onChange(value[0])} max={5} step={1} /></FormControl>
        </FormItem>
    )}/>
);

const AdminEvaluationDialog = ({ studentId, onEvaluationSubmit }: { studentId: string, onEvaluationSubmit: () => void }) => {
    const { user } = useRole();
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild><Button><Award className="mr-2 h-4 w-4" /> Submit Final Evaluation</Button></DialogTrigger>
            <DialogContent className="sm:max-w-2xl">
                <DialogHeader><DialogTitle>Final Admin Evaluation</DialogTitle><DialogDescription>Provide the final assessment for this student based on all available data.</DialogDescription></DialogHeader>
                <Form {...form}>
    ...
    return (
        <div className="space-y-6">
            <Card>
                <CardHeader>
                    <div className="flex justify-between items-center">
                        ...
                        {!adminEval && <AdminEvaluationDialog studentId={studentId} onEvaluationSubmit={refreshData} />}
                    </div>
                </CardHeader>
            </Card>
            {adminEval && <EvaluationCard title="Final Admin Review" icon={Award} evaluation={adminEval} />}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {lecturerEval ? <EvaluationCard title="Lecturer Evaluation" icon={UserIcon} evaluation={lecturerEval} /> : <Card className="flex items-center justify-center flex-col min-h-[300px] text-center"><CardContent className="pt-6"><p className="text-muted-foreground">Lecturer evaluation has not been submitted yet.</p></CardContent></Card>}
            </div>
        </div>
    )
};
...
export default function StudentDetailPage({ params }: { params: { studentId: string } }) {
    ...
    if (error || !details) {
        return (
            <Card>
                ...
                <CardContent><Button asChild variant="outline" className="mt-4"><Link href="/admin/students"><ArrowLeft className="mr-2 h-4 w-4" />Back to Students</Link></Button></CardContent>
            </Card>
        );
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-4">
                <Button asChild variant="outline" size="icon"><Link href="/admin/students"><ArrowLeft className="h-4 w-4" /></Link></Button>
