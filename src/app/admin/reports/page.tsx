...
import { getAllReports, approveReport, rejectReport, type Report } from '@/services/reportsService';
...
export default function AdminReportsPage() {
    ...
    const fetchReports = async (tab: 'pending' | 'history') => {
        ...
        try {
            ...
            if (tab === 'pending') {
                setReports(allReports.filter(r => r.status === 'Pending') as ReportWithStudentName[]);
            } else {
                setReports(allReports.filter(r => r.status !== 'Pending') as ReportWithStudentName[]);
            }
        } catch (error) {
        ...
        if (reportList.length === 0) {
            return (
                <div className="text-center text-muted-foreground py-10">
                    <p>
                        {activeTab === 'pending' 
                            ? "No reports are currently pending review. Well done!"
                            : "No reports in the review history."}
                    </p>
                </div>
            );
        }
                        ...
                        <AccordionItem value={report.id} key={report.id} className="border rounded-lg px-4 bg-muted/20">
                            ...
                            <AccordionContent className="space-y-4 pt-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground whitespace-pre-wrap">{report.fullReport}</p>
                                </div>
                                {report.summary && <div>
                        ...
                        <div className="flex justify-between w-full items-start">
                         ...
                         <div className="mt-2 pt-2 border-t">
                            <p className="text-sm text-muted-foreground whitespace-pre-wrap">
                                <span className="font-semibold">Feedback:</span> {report.lecturerComment}
                            </p>
                        </div>
                    </li>
                ))}
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Review all daily reports submitted by students. Provide feedback, approve, or reject them.</CardDescription>
            </CardHeader>
            <CardContent>
